function MusVolumeTS = ExtractMusVolumesFromNodesAndLinksVols2( Muses, NodeVolumeFile, LinksVolumenFile )
%EXTRACTVOLUMESFROM Summary of this function goes here
%   Detailed explanation goes here
disp('ExtractMusVolumesFromNodesAndLinksVols2')
...
for(i=1:Nnodes)
    for(j=1:Nmus)
        if(Muses(j).isNodeInMus(strtrim(MUIDs{i})))
        node2volMap(i) = j;
        break;
        end
    end
...
while ~feof(fid)
    ...
    for(i=1:Nnodes)
        index = node2volMap(i);
        VolTS.data(ti,index) = VolTS.data(ti,index) + str2double(split{i+1});
    end
...
for(i=1:Nlinks)
    for(j=1:Nmus)
        if(Muses(j).isLinkInMus(strtrim(MUIDs{i})))
        links2volMap(i) = j;
        break;
        end
    end
